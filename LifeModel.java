...
public class LifeModel implements ActionListener
{
    	...
    	for (int r = 0; r < SIZE; r++) {
            for (int c = 0; c < SIZE; c++) {
                ...
                if (!myGrid[r][c].isAliveNow()) {
                    //use the neighbor amount to see if cell will come alive
                    if (numLiveNeighbors(r, c) == 3) {
                        myGrid[r][c].setAliveNext(true);
                    } else {
                        myGrid[r][c].setAliveNext(false);
                    }
                } else {
                    //use neighbor amount to see if cell will die
                    if (numLiveNeighbors(r, c) < 2 || numLiveNeighbors(r, c) > 3) {
                        myGrid[r][c].setAliveNext(false);
                    } else {
                        myGrid[r][c].setAliveNext(true);
                    }
    ...
    {
        int lNeighborsNum = 0;
        //iterate through 3x3 grid around a certain cell
        for (int r = row - 1; r < row + 2; r++) {
            for (int c = col - 1; c < col + 2; c++) {
                //if it is in bounds, alive, and not the current cell:
                // add to the live neighbor count
                if (inBounds(r, c)) {
                    if (myGrid[r][c].isAliveNow() && (myGrid[r][c] != myGrid[row][col])) {
                        lNeighborsNum++;
                    }
                }
            }
        }
        return lNeighborsNum;
    }
